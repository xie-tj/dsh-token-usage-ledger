//#region lib/types/host/invariant.js
...
const name = "usage-ledger-invariant";
...
function applyAttempt(open, event, fail) {
	...
	if (start.turn !== event.data.turn || start.step !== event.data.step || start.provider !== event.data.provider || start.model !== event.data.model) fail(`request attempt '${id}' ends with fields different from its start`);
	open.delete(id);
}
...
const install = Object.assign((ctx, fail) => {
	...
	ctx.on("session/event", (session, event) => {
		applyAttempt(stateFor(session), event, fail);
	}, { global: true });
}, { inject: ["sessions"] });
