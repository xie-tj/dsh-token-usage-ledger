...
var UsageLedgerGovernor = class {
	...
	observe(sample) {
		if (this.config.powerMode === "ac-only" && sample.powerSource !== "ac") return this.paused("battery");
		if (sample.rssMiB >= this.config.pauseRssMiB || this.config.pauseAvailableMemoryMiB > 0 && sample.availableMemoryMiB !== void 0 && sample.availableMemoryMiB <= this.config.pauseAvailableMemoryMiB) return this.paused("memory");
		if (sample.eventLoopUtilization >= this.config.pauseEventLoopUtilization || sample.eventLoopDelayMs >= this.config.pauseEventLoopDelayMs) return this.paused("event-loop");
	...
	current(mode, reason) {
		return {
			mode,
			delayMs: mode === "pause" ? this.config.maxDelayMs : Math.min(this.config.maxDelayMs, Math.max(0, Math.ceil(this.config.sliceMs * (1 / this.workShare - 1)))),
...
const DEFAULT_INITIAL_WORK_SHARE = .1;
const DEFAULT_MIN_WORK_SHARE = .05;
const DEFAULT_MAX_WORK_SHARE = .5;
const DEFAULT_AIMD_INCREASE = .05;
const DEFAULT_AIMD_DECREASE = .5;
...
function resolveConfig(config) {
	...
	if (!Number.isFinite(adaptive.multiplicativeDecrease) || adaptive.multiplicativeDecrease <= 0 || adaptive.multiplicativeDecrease >= 1) throw new RangeError("usage ledger backfillAimdDecrease must be greater than 0 and less than 1");
	if (adaptive.minWorkShare > adaptive.initialWorkShare || adaptive.initialWorkShare > adaptive.maxWorkShare) throw new RangeError("usage ledger work shares must satisfy min <= initial <= max");
...
let UsageLedgerService = (() => {
	...
	return class UsageLedgerService extends _classSuper {
		...
		static Config = z.object({
			...
			backfillInitialWorkShare: z.number().min(.001).max(1).default(DEFAULT_INITIAL_WORK_SHARE),
			backfillMinWorkShare: z.number().min(.001).max(1).default(DEFAULT_MIN_WORK_SHARE),
			backfillMaxWorkShare: z.number().min(.001).max(1).default(DEFAULT_MAX_WORK_SHARE),
			backfillAimdIncrease: z.number().min(.001).max(1).default(DEFAULT_AIMD_INCREASE),
			backfillAimdDecrease: z.number().min(.001).max(.999).default(DEFAULT_AIMD_DECREASE),
