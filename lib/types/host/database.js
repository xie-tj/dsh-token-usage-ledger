...
export async function openUsageLedgerDatabase(path) {
...
function configureDatabase(db, path) {
    ...
    if (applicationId.application_id === USAGE_LEDGER_SQLITE_APPLICATION_ID && version.user_version !== USAGE_LEDGER_SQLITE_SCHEMA_VERSION) {
        throw new Error(`usage ledger database at "${path}" has schema ${String(version.user_version)}, expected ${String(USAGE_LEDGER_SQLITE_SCHEMA_VERSION)}`);
    }
...
function callFromSql(row) {
    return {
        ...
        ...(decodeUsage(row.provisional_input_tokens, row.provisional_output_tokens, row.provisional_cache_read_tokens, row.provisional_cache_write_tokens) === undefined ? {} : {
            provisionalUsage: decodeUsage(row.provisional_input_tokens, row.provisional_output_tokens, row.provisional_cache_read_tokens, row.provisional_cache_write_tokens),
        }),
        ...(decodeUsage(row.final_input_tokens, row.final_output_tokens, row.final_cache_read_tokens, row.final_cache_write_tokens) === undefined ? {} : {
            finalUsage: decodeUsage(row.final_input_tokens, row.final_output_tokens, row.final_cache_read_tokens, row.final_cache_write_tokens),
        }),
    };
}
  ...
  `);
    ...
    `).get(sessionId, createdAt);
        ...
        for (const attemptId of attemptIds) {
            ...
            if (row !== undefined)
                calls.push({ key: row.key, row: callFromSql(row) });
        }
        return { cursor, calls };
    }
