...
import { openUsageLedgerDatabase } from "./database.js";
import { UsageLedgerGovernor } from "./governor.js";
import { USAGE_LEDGER_WORKER_PROTOCOL } from "./worker-protocol.js";
import { UsageWorkerSupervisor } from "./supervisor.js";
...
export const USAGE_LEDGER_SETTINGS_NAMESPACE = 'usage-ledger';
...
function resolveConfig(config) {
    ...
    if (backfillMode !== 'process' && backfillMode !== 'off')
        throw new RangeError('usage ledger backfillMode is invalid: ' + String(backfillMode));
    if (backfillScope !== 'all' && backfillScope !== 'recent')
        throw new RangeError('usage ledger backfillScope is invalid: ' + String(backfillScope));
    if (adaptive.powerMode !== 'ac-only' && adaptive.powerMode !== 'always')
        throw new RangeError('usage ledger backfillPowerMode is invalid: ' + String(adaptive.powerMode));
    ...
    if (adaptive.minDelayMs > adaptive.maxDelayMs)
        throw new RangeError('usage ledger backfillMinDelayMs must not exceed backfillMaxDelayMs');
    if (adaptive.busyEventLoopUtilization > adaptive.pauseEventLoopUtilization)
        throw new RangeError('usage ledger busy utilization must not exceed pause utilization');
    if (adaptive.busyEventLoopDelayMs > adaptive.pauseEventLoopDelayMs)
        throw new RangeError('usage ledger busy event-loop delay must not exceed pause delay');
    return {
        databasePath,
        backfillMode,
        backfillScope,
...
function zoneOffset(time, timeZone) {
    ...
    const number = (kind) => {
        ...
        if (value === undefined)
            throw new Error('usage ledger could not resolve time zone part ' + kind);
        return Number(value);
    };
...
function zoneDayStart(day, timeZone) {
    ...
    if (year === undefined || month === undefined || date === undefined)
        throw new Error('usage ledger received invalid day ' + day);
...
function resolveSnapshotRequest(request) {
    ...
    for (const [name, value] of [['workspace', workspace], ['provider', provider], ['model', model]]) {
        if (value !== null && typeof value !== 'string')
            throw new TypeError('usage ledger ' + name + ' must be a string or null');
    }
    if (typeof timeZone !== 'string' || timeZone.length === 0)
        throw new TypeError('usage ledger timeZone must be a non-empty IANA timezone');
    if (typeof all !== 'boolean')
        throw new TypeError('usage ledger all must be a boolean');
    try {
    ...
    if (!all && (!Number.isSafeInteger(days) || days < 1 || days > MAX_DAYS))
        throw new RangeError('usage ledger days must be a safe integer from 1 through ' + String(MAX_DAYS));
...
function compactEvent(event) {
    ...
    switch (event.type) {
        ...
        case 'llm/retry':
            return { ...base, type: event.type, data: { turn: event.data.turn, step: event.data.step } };
        ...
        case 'turn/end':
            return { ...base, type: event.type, data: { turn: event.data.turn, reason: { kind: event.data.reason.kind } } };
...
let UsageLedgerService = (() => {
    ...
    return class UsageLedgerService extends _classSuper {
        static {
            ...
            __esDecorate(this, null, _statusSnapshot_decorators, { kind: "method", name: "statusSnapshot", static: false, private: false, access: { has: obj => "statusSnapshot" in obj, get: obj => obj.statusSnapshot }, metadata: _metadata }, null, _instanceExtraInitializers);
        ...
        async [Service.init]() {
            ...
            this.ctx.on('session/event', (session, event) => {
                ...
                if (compact !== undefined)
                    this.worker.sendLive({ type: 'live', session: workerSession(session), event: compact });
            }, { global: true });
            this.ctx.on('session/disposed', session => {
                this.worker.sendControl({ type: 'dispose', session: workerSession(session) }, session.id);
            }, { global: true });
            if (this.resolvedConfig.backfillMode === 'off')
                return;
            this.startGovernor();
        ...
        async startWorker(persistence) {
            ...
            try {
                ...
                if (readerSpec === undefined)
                    this.ctx.logger.warn('usage ledger: persistence has no provider-owned background reader; live ledger remains enabled');
        ...
        async snapshot(request) {
            ...
            do {
                ...
                for (const { row } of page.rows) {
                    ...
                    if (localDay < fromDay || localDay > resolved.throughDay)
                        continue;
                    if (selected.length < this.resolvedConfig.snapshotEventLimit)
                        selected.push(row);
                    else
                    ...
                    if (day !== undefined)
                        daily.set(localDay, addAttempt(day, row));
                }
                after = page.next;
                if (after !== undefined)
        ...
        async exportCsv(request) {
            ...
            try {
                ...
                do {
                    ...
                    for (const { row } of page.rows) {
                        const localDay = zoneDay(row.startedAt, resolved.timeZone);
                        if (localDay < range.fromDay || localDay > resolved.throughDay)
                            continue;
                        const usage = row.finalUsage ?? row.provisionalUsage;
                        await writeCsv(stream, [
        ...
        async sampleLoad() {
            ...
            if (this.pace?.mode === next.mode && this.pace.delayMs === next.delayMs && this.pace.reason === next.reason)
                return;
            this.pace = next;
        ...
        requireDatabase() {
            if (this.database === undefined)
                throw new Error('usage ledger is not initialized');
            return this.database;
        }
    };
})();
export { UsageLedgerService };
