...
import { usageLedgerDomainSpec } from "./spec.js";
...
let UsageLedgerService = (() => {
    ...
    return class UsageLedgerService extends _classSuper {
        ...
        async snapshot(request) {
            ...
            for (const [, row] of this.requireCalls().entries()) {
                ...
                if (localDay < fromDay || localDay > resolved.throughDay)
                    continue;
                if (resolved.workspace !== null && row.workspace !== resolved.workspace)
                    continue;
                selected.push(row);
        ...
        scheduleWrites() {
            if (this.writeScheduled || this.writing)
                return;
            ...
            try {
                ...
                for (let index = 0; index < callBatch.length; index += 1) {
                    ...
                    try {
                        ...
                        for (const [remainingKey, remainingRow] of callBatch.slice(index))
                            this.pendingCalls.set(remainingKey, remainingRow);
                ...
                for (let index = 0; index < cursorBatch.length; index += 1) {
                    ...
                    try {
                        ...
                        for (const [remainingKey, remainingRow] of cursorBatch.slice(index))
                            this.pendingCursors.set(remainingKey, remainingRow);
                ...
                for (let index = 0; index < deleteBatch.length; index += 1) {
                    ...
                    try {
                        ...
                        for (const [remainingKey, remainingCreatedAt] of deleteBatch.slice(index))
                            this.pendingDeletes.set(remainingKey, remainingCreatedAt);
