...
async function openHistorical(persistence, descriptor) {
    ...
    if (typeof runtime.open === 'function') {
        ...
        try {
            ...
            if (header.id !== descriptor.id)
                throw new TypeError('usage ledger session metadata id mismatch');
            ...
            return {
                ...
                read: async (offset = 0, length = HISTORICAL_READ_BATCH_SIZE) => {
                    ...
                    if (!Array.isArray(events))
                        throw new TypeError('usage ledger received an invalid session event batch');
                    return events;
                },
                close: closeOnce,
            };
    ...
    if (typeof runtime.inspect !== 'function')
        throw new TypeError('usage ledger persistence has no historical reader');
...
let UsageLedgerService = (() => {
    ...
    return class UsageLedgerService extends _classSuper {
        ...
        cursors = new Map();
        routes = new Map();
        coldSessions = new Set();
        tails = new Map();
        pendingThrough = new Map();
        scheduled = new Set();
        backfillPromise;
        accepting = true;
        constructor(ctx) {
        ...
        async [Service.init]() {
            ...
            this.ctx.on('session/disposed', (session) => {
                this.enqueue(session, async () => {
                    ...
                    if (sameLifecycle(row, session))
                        await this.requireSessions().delete(session.id);
                });
            }, { global: true });
            ...
            for (const snapshot of snapshots) {
                ...
                try {
                    ...
                    try {
                        ...
                        const historical = { id, header: handle.header, inheritedEventCount: handle.inheritedEventCount };
                        ...
                        try {
                            ...
                            if (current.observedSeq >= 0)
                                this.cursors.set(historical, current);
        ...
        async snapshot(request) {
            if (this.backfillPromise === undefined)
                this.backfillPromise = this.backfill(this.ctx.sessionPersistence);
            await this.backfillPromise;
            for (const session of this.ctx.sessions.list())
                this.adopt(session);
        ...
        schedule(session, throughSeq) {
            ...
            if (pending === undefined || throughSeq > pending)
                this.pendingThrough.set(session, throughSeq);
            if (this.scheduled.has(session))
                return;
            ...
            this.enqueue(session, async () => {
                ...
                try {
                    ...
                    if (this.pendingThrough.get(session) === target)
                        this.pendingThrough.delete(session);
                }
                finally {
                    const next = this.pendingThrough.get(session);
                    if (next !== undefined && this.accepting)
                        this.schedule(session, next);
                }
            });
        }
        ...
        async waitForSession(session) {
            while (true) {
                ...
                await tail;
                if (this.tails.get(session) === tail && !this.pendingThrough.has(session))
                    return;
            }
        }
        ...
        async processThrough(session, throughSeq) {
            ...
            if (current.observedSeq !== initial.observedSeq)
                this.cursors.set(session, current);
        }
        ...
        async recordFinish(session, current, event, _route, kind) {
            ...
            if (row.outcome === undefined)
                await this.requireCalls().put(key, { ...row, outcome });
            return current;
        }
        ...
        async processRetry(session, current, event) {
            ...
            const row = this.requireCalls().get(key);
            if (row !== undefined) {
                await this.requireCalls().put(key, { ...row, ...(row.outcome === undefined ? { outcome: 'failure' } : {}), retryScheduled: true });
            }
            return current;
        }
        ...
        async replaceFinalUsage(session, current, turn, step, usage) {
            ...
            const row = this.requireCalls().get(key);
            if (row !== undefined)
                await this.requireCalls().put(key, { ...row, finalUsage: usageOf(usage) });
            return current;
        }
