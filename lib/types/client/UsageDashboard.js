...
function selectedEvents(events, model, period, throughDay) {
    ...
    if (period === 'all' || byModel.length === 0)
        return byModel;
...
export function UsageDashboard({ readSnapshot, t }) {
    ...
    return (_jsxs("section", { className: css.section, "aria-busy": state.status === 'loading', children: [_jsxs("header", { className: css.header, children: [_jsxs("div", { children: [_jsx("p", { className: css.eyebrow, children: "API / LEDGER" }), _jsx("h2", { children: t('title') }), _jsx("p", { className: css.intro, children: t('intro') })] }), _jsx("button", { type: "button", className: css.refresh, disabled: state.status === 'loading', onClick: refresh, children: state.status === 'loading' ? t('refreshing') : t('refresh') })] }), state.status === 'error' ? (_jsx("p", { className: css.stale, role: "status", children: t('showingLastGood') })) : null, _jsx("p", { className: css.updated, children: interpolate(t('updated'), { time: snapshot.updatedAt }) }), _jsxs("div", { className: css.filters, children: [_jsxs("label", { children: [_jsx("span", { children: t('model') }), _jsxs("select", { value: model, onChange: (event) => { setModel(event.currentTarget.value); }, children: [_jsx("option", { value: "all", children: t('allModels') }), models.map(row => _jsx("option", { value: row.model, children: row.model }, row.model))] })] }), _jsxs("label", { children: [_jsx("span", { children: t('period') }), _jsxs("select", { value: period, onChange: (event) => { setPeriod(event.currentTarget.value); }, children: [_jsx("option", { value: "7d", children: t('sevenDays') }), _jsx("option", { value: "30d", children: t('thirtyDays') }), _jsx("option", { value: "all", children: t('allTime') })] })] })] }), events.length === 0 ? _jsx("p", { className: css.empty, children: t('noData') }) : (_jsxs(_Fragment, { children: [_jsxs("div", { className: css.metrics, children: [_jsx(Metric, { className: css.metricTotal, label: t('totalTokens'), value: tokenText(totals.input + totals.output + totals.cached) }), _jsx(Metric, { label: t('requests'), value: fullNumberText(totals.requests) }), _jsx(Metric, { label: t('inputTokens'), value: tokenText(totals.input) }), _jsx(Metric, { label: t('outputTokens'), value: tokenText(totals.output) }), _jsx(Metric, { label: t('unmeteredRequests'), value: fullNumberText(totals.unmetered) }), _jsx(Metric, { label: t('failedRequests'), value: fullNumberText(totals.failed) }), _jsx(Metric, { label: t('retryRequests'), value: fullNumberText(totals.retried) })] }), _jsxs("div", { className: css.charts, children: [_jsxs("article", { className: css.chartCard, children: [_jsxs("div", { className: css.chartHeading, children: [_jsx("h3", { children: t('requestCurve') }), _jsx("span", { children: fullNumberText(totals.requests) })] }), _jsxs("div", { className: css.curveChart, children: [_jsxs("svg", { viewBox: "0 0 100 40", preserveAspectRatio: "none", "aria-hidden": "true", children: [_jsx("path", { className: css.gridLine, d: "M 0 36 H 100" }), _jsx("path", { className: css.curve, d: curve }), buckets.map((bucket, index) => {
                                                        ...
                                                        return _jsx("circle", { className: css.curvePoint, cx: x, cy: y, r: "1.3" }, bucket.date);
                                                    ...
                                                    return (_jsx("button", { type: "button", "aria-describedby": target?.kind === 'requests' && target.index === index ? tooltipId : undefined, "aria-label": interpolate(t('requestsOn'), { date: dateText(bucket.date), requests: fullNumberText(bucket.requests), failed: fullNumberText(bucket.failed), retried: fullNumberText(bucket.retried) }), onFocus: () => { showTarget(next); }, onPointerEnter: () => { showTarget(next); }, onPointerLeave: () => { setTarget(undefined); }, onClick: () => { toggleTarget(next); } }, bucket.date));
                                    ...
                                    ? _jsx("span", { children: interpolate(t('requestsOn'), { date: '', requests: fullNumberText(activeBucket.requests), failed: fullNumberText(activeBucket.failed), retried: fullNumberText(activeBucket.retried) }).replace(/^：|^: /, '') })
                                    : _jsx("span", { children: interpolate(t('tokensOn'), { date: '', input: tokenText(activeBucket.input), output: tokenText(activeBucket.output), cached: tokenText(activeBucket.cached) }).replace(/^：|^: /, '') })] })) }), _jsxs("article", { className: css.tableCard, children: [_jsxs("div", { className: css.chartHeading, children: [_jsx("h3", { children: t('modelBreakdown') }), _jsx("span", { children: visibleModels.length })] }), _jsx("div", { className: css.tableScroll, children: _jsxs("table", { children: [_jsx("thead", { children: _jsxs("tr", { children: [_jsx("th", { children: t('tableModel') }), _jsx("th", { children: t('tableRequests') }), _jsx("th", { children: t('tableFailed') }), _jsx("th", { children: t('tableRetries') }), _jsx("th", { children: t('tableInput') }), _jsx("th", { children: t('tableOutput') }), _jsx("th", { children: t('tableTotal') })] }) }), _jsx("tbody", { children: visibleModels.filter(row => model === 'all' || row.model === model).map(row => (_jsxs("tr", { children: [_jsx("th", { scope: "row", children: row.model }), _jsx("td", { children: fullNumberText(row.requests) }), _jsx("td", { children: fullNumberText(row.failed) }), _jsx("td", { children: fullNumberText(row.retried) }), _jsx("td", { children: tokenText(row.input) }), _jsx("td", { children: tokenText(row.output) }), _jsx("td", { children: tokenText(totalOf(row)) })] }, row.model))) })] }) })] })] }))] }));
}
