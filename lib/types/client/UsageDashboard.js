...
export function UsageDashboard({ readSnapshot, t }) {
    ...
    const providers = useMemo(() => [...new Set(models.map(row => row.provider))].sort(), [models]);
    const modelOptions = useMemo(() => [...new Set(models
            .filter(row => provider === 'all' || row.provider === provider)
            .map(row => row.model))].sort(), [models, provider]);
    const events = useMemo(() => snapshot === undefined ? [] : selectedEvents(snapshot.events, provider, model, period, snapshot.throughDay), [model, period, provider, snapshot]);
    const buckets = useMemo(() => snapshot === undefined ? [] : bucketsOf(events, period, snapshot.daily, snapshot.throughDay, provider === 'all' && model === 'all'), [events, model, period, provider, snapshot]);
    const visibleModels = useMemo(() => [...aggregateModels(events, showProvider)].sort((left, right) => totalOf(right) - totalOf(left)), [events, showProvider]);
