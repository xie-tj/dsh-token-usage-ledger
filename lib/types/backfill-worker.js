...
async function processSession(task) {
    ...
    if (reader !== undefined && !stopped) {
        const batches = reader.readSessionBatches(init.readerSpec?.options, { session: { id: session.id, ...(session.cwd === undefined ? {} : { cwd: session.cwd }) }, fromSeq: 0, batchEvents: init.config.workerBatchEvents });
