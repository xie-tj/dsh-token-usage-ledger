...
function applyAttempt(open: Map<string, OpenAttempt>, event: SessionEvent, fail: InvariantFailure): void {
  if (event.type !== 'llm/request-attempt') return
  const id = event.data.attemptId as string
...
const install: InvariantInstaller = Object.assign((ctx: Context, fail: InvariantFailure) => {
  const states = new WeakMap<Session, Map<string, OpenAttempt>>()
  const seed = (session: Session): Map<string, OpenAttempt> => {
  ...
  const stateFor = (session: Session): Map<string, OpenAttempt> => states.get(session) ?? seed(session)
